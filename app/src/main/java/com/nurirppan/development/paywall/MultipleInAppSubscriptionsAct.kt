...
class MultipleInAppSubscriptionsAct : AppCompatActivity(), PurchasesUpdatedListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Log.i("Act__", "MultipleInAppSubscriptionsAct")

        setContentView(R.layout.activity_multiple_in_app_subscriptions)
            ...
            billingClient!!.querySkuDetailsAsync(
                ...
                if (billingResult.responseCode == BillingClient.BillingResponseCode.OK) {
                    if (skuDetailsList != null && skuDetailsList.size > 0) {
                        ...
                        Toast.makeText(
                            applicationContext,
                            "Subscribe Item $PRODUCT_ID not Found",
                            Toast.LENGTH_SHORT
                        ).show()
                    }
                } else {
        ...
        for (purchase in purchases) {
            ...
            if (index > -1) {
                ...
                if (purchase.purchaseState == Purchase.PurchaseState.PURCHASED) {
                    ...
                    if (!purchase.isAcknowledged) {
                        ...
                        billingClient!!.acknowledgePurchase(
                            ...
                            if (billingResult.responseCode == BillingClient.BillingResponseCode.OK) {
                                ...
                                Toast.makeText(
                                    applicationContext,
                                    subcribeItemIDs[index] + " Item Subscribed",
                                    Toast.LENGTH_SHORT
                                ).show()
                                notifyList()
                            }
                    ...
                    Toast.makeText(
                        applicationContext,
                        subcribeItemIDs[index] + " Purchase is Pending. Please complete Transaction",
                        Toast.LENGTH_SHORT
                    ).show()
                    ...
                    Toast.makeText(
                        applicationContext,
                        subcribeItemIDs[index] + " Purchase Status Unknown",
                        Toast.LENGTH_SHORT
                    ).show()
                    notifyList()
                }
    ...
    private fun verifyValidSignature(signedData: String, signature: String): Boolean {
        return try {
            ...
            MultipleInAppSubscriptionsSecurity.verifyPurchase(base64Key, signedData, signature)
        } catch (e: IOException) {
            false
        }
    }
