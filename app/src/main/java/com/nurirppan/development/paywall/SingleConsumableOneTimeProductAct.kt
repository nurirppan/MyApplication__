...
class SingleConsumableOneTimeProductAct : AppCompatActivity(), PurchasesUpdatedListener {

    private lateinit var binding: ActivitySingleConsumableOneTimeProductBinding

    private var billingClient: BillingClient? = null
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)

        Log.i("Act__", "SingleConsumableOneTimeProductAct")

        binding = ActivitySingleConsumableOneTimeProductBinding.inflate(layoutInflater)
        ...
        binding.consumeCount!!.text = "Item Consumed $purchaseCountValueFromPref Time(s)"
    ...
    private fun verifyValidSignature(signedData: String, signature: String): Boolean {
        return try {
            ...
            SingleConsumableOneTimeProductSecurity.verifyPurchase(base64Key, signedData, signature)
        } catch (e: IOException) {
            false
        }
    }
    ...
    companion object {
        ...
        const val PURCHASE_KEY = "SingleConsumableOneTimeProduct"
        const val PRODUCT_ID = "singlec_onsumable_product_id"
    }
}